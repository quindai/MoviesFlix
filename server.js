...
app.get("/unified/:num", async function (req, res) {
  var data = [];
  movieCtrl.listLimit(req.params.num, function (resp) {
    data.push(resp);
  });
...
app.get("/tvshows/:num", async function (req, res) {
  //res.end("Pegando usuario com esse ID");
  tvshowCtrl.listLimit(req.params.num, function (resp) {
    res.send(resp);
  });
...
app.put(
  "/movies/:images:/:comment/:sinopse/:release:/:stars/:categories/:duracao/:atores/:trailer:/:lancamento",
  function (req, res) {
