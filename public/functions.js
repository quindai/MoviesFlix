...
var radio_control = function () {
  ...
  document.getElementById("control-temps").style.display =
    value == "/movies" ? "none" : "block";
};
...
var filterCt = function () {
  ...
  //alert("funcionando");
  $.get(
    document.querySelector('input[name="opts"]:checked').value,
    function (data, status) {
      ...
      for (i = 0; i < data.length; ++i) {
        $("#main").append(
          '<div id="movies_main">' + preenche(data[i]) + "</div>"
        );
      }
    }
  );
...
var preenche = function (data) {
  return (
    '<div class="col-sm-6 col-md-3 thumbs">' +
    '<a href="details.html?title=' +
    data.title +
    "&poster=" +
    ...
    "</h3>" +
    '<div class="sinopse">' +
    '<p class="block-with-text" style="width: 100%;">' +
    data.sinopse +
    "</p>" +
    "</div>" +
...
var pesquisar = function (form, url, method) {
  ...
  myhttpRequest.onreadystatechange = function () {
    if (myhttpRequest.readyState == 4 && myhttpRequest.status == 200) {
      ...
      $("#main").append(
        '<div class="row searchR" ><h2>Resultado da Pesquisa</h2></div><hr>'
      );
...
var yolo = function (url) {
  //preenche a tela com filmes, series ou tvshows apenas
  var acao = "ACAO";
  $.get(url, function (data, status) {
    document.getElementById("main").innerHTML = ""; //limpa tela
    $("#main").append(
      '<div class="row searchR" ><div class="btn-group" data-toggle="buttons" style="padding-left: 15%;">' +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option1" autocomplete="off" value="' +
        url +
        '">TODOS' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option2" autocomplete="off" value="' +
        url +
        '/search/categories/AÇÃO">AÇÃO' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option3" autocomplete="off" value="' +
        url +
        '/search/categories/SCI-FI">SCI-FI' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option4" autocomplete="off" value="' +
        url +
        '/search/categories/ROMANCE">ROMANCE' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option5" autocomplete="off" value="' +
        url +
        '/search/categories/COMÉDIA">COMÉDIA' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option5" autocomplete="off" value="' +
        url +
        '/search/categories/THRILLER">THRILLER' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option5" autocomplete="off" value="' +
        url +
        '/search/categories/DRAMA">DRAMA' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option6" autocomplete="off" value="' +
        url +
        '/search/categories/AVENTURA">AVENTURA' +
        "</label>" +
        '<label class="btn btn-success">' +
        '<input type="radio" name="opts" id="option7" autocomplete="off" value="' +
        url +
        '/search/categories/ANIMAÇÃO">ANIMAÇÃO' +
        "</label>" +
        "</div><hr>" +
...
var gravar = function (form) {
  //inspired by - http://help.dottoro.com/ljppxrti.php
  ...
  try {
    ...
    myhttpRequest.setRequestHeader(
      "Content-Type",
...
function OnReadyStateChanged(myhttpRequest, form) {
  if (myhttpRequest.readyState == 0 || myhttpRequest.readyState == 4) {
    ...
    var success =
      myhttpRequest.status == 0 || //compatibilidade com IE
      (myhttpRequest.status >= 200 && myhttpRequest.status < 300) ||
      myhttpRequest.status == 304 ||
      myhttpRequest.status == 1223;
      ...
      if (nodeName === "select") {
        for (var j = 0; j < elem.options.length; j++) {
          ...
          if (option.selected) {
            ...
            var value =
              valueAttr && valueAttr.specified ? option.value : option.text;
            if (param != "") {
              param += "&";
            }
            param +=
              encodeURIComponent(elem.name) + "=" + encodeURIComponent(value);
          }
        }
      } else {
        param =
